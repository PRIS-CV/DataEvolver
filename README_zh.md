...
├── scripts/                           # 公共 onboarding 与工作流入口
│   ├── bootstrap_dataevolver_default.sh      # dry-run 配置规划
│   ├── dataevolver_production.py             # 生产 doctor/runtime CLI
│   ├── run_scene_agent_monitor.py           # VLM 闭环 Agent 监控
│   ├── stage0_web_research.py               # WebSearch prior 与 handoff 生成
│   ├── run_hyworld_scene_pano.py            # HYWorld panorama 桥接
│   ├── run_hyworld_full_worldgen.py         # HYWorld full worldgen 桥接
│   ├── universal_3d_layout/                 # Universal 3D 数据集工作流
│   └── ...                                  # 其他已文档化公共入口
├── tests/                             # 公共 smoke 与契约测试
├── assets/
│   ├── hdri/                                # HDRI 环境贴图
