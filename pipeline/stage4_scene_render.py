...
if _sys.version_info[:2] == (3, 10):
    _np310_path = "<blender-addon-modules>"
...
def load_scene_template(path: str) -> dict:
    ...
    data.setdefault("blend_path", "<scene-blend-path>")
...
def resolve_material_hsv_adjustment(control_material: dict, has_image_texture: bool, effective_value_scale: float) -> dict:
    raw_saturation_scale = float(control_material.get("saturation_scale", 1.0))
    raw_value_scale = float(effective_value_scale)
    raw_hue_offset = float(control_material.get("hue_offset", 0.0))
    allow_textured_hsv = bool(control_material.get("allow_textured_hsv_adjustment", False))
    if has_image_texture and not allow_textured_hsv:
        return {
            "saturation_scale": 1.0,
            "value_scale": 1.0,
            "hue_offset": 0.0,
            "raw_saturation_scale": round(raw_saturation_scale, 4),
            "raw_value_scale": round(raw_value_scale, 4),
            "raw_hue_offset": round(raw_hue_offset, 4),
            "preserved_textured_hsv": True,
            "allow_textured_hsv_adjustment": False,
            "reason": "textured_mesh_hsv_preserved",
        }
    return {
        "saturation_scale": raw_saturation_scale,
        "value_scale": raw_value_scale,
        "hue_offset": raw_hue_offset,
        "raw_saturation_scale": round(raw_saturation_scale, 4),
        "raw_value_scale": round(raw_value_scale, 4),
        "raw_hue_offset": round(raw_hue_offset, 4),
        "preserved_textured_hsv": False,
        "allow_textured_hsv_adjustment": allow_textured_hsv,
        "reason": "control_material_hsv_applied",
    }
...
def render_object(asset_path: str, obj_id: str, output_dir: str,
    ...
    has_image_texture = any(_object_has_image_texture(obj) for obj in mesh_objects)
    if has_image_texture:
        template_value_scale = 1.0
        template_roughness_add = 0.0
        template_specular_add = 0.0
    else:
    ...
    raw_control_roughness_add = float(control_material.get("roughness_add", 0.0))
    raw_control_specular_add = float(control_material.get("specular_add", 0.0))
    allow_textured_surface = bool(control_material.get("allow_textured_surface_adjustment", False))
    if has_image_texture and not allow_textured_surface:
        control_roughness_add = 0.0
        control_specular_add = 0.0
        surface_adjustment = {
            "raw_roughness_add": round(raw_control_roughness_add, 4),
            "raw_specular_add": round(raw_control_specular_add, 4),
            "roughness_add": 0.0,
            "specular_add": 0.0,
            "preserved_textured_surface": True,
            "allow_textured_surface_adjustment": False,
            "reason": "textured_mesh_surface_preserved",
        }
    else:
        control_roughness_add = raw_control_roughness_add
        control_specular_add = raw_control_specular_add
        surface_adjustment = {
            "raw_roughness_add": round(raw_control_roughness_add, 4),
            "raw_specular_add": round(raw_control_specular_add, 4),
            "roughness_add": round(control_roughness_add, 4),
            "specular_add": round(control_specular_add, 4),
            "preserved_textured_surface": False,
            "allow_textured_surface_adjustment": allow_textured_surface,
            "reason": "control_material_surface_applied",
        }
    ...
    hsv_adjustment = resolve_material_hsv_adjustment(
        control_material,
        has_image_texture,
        effective_value_scale,
    )
    ...
    material_adaptation = {
        "applied": False,
        "used_mode": None,
        "had_image_texture": has_image_texture,
        "forced_override": False,
    ...
    if not should_preserve_materials:
        ...
        hsv_adjusted_materials = adjust_material_hsv(
            mesh_objects,
            saturation_scale=float(hsv_adjustment["saturation_scale"]),
            value_scale=float(hsv_adjustment["value_scale"]),
            hue_offset=float(hsv_adjustment["hue_offset"]),
        )
        ...
        material_adaptation["raw_control_roughness_add"] = round(raw_control_roughness_add, 4)
        material_adaptation["raw_control_specular_add"] = round(raw_control_specular_add, 4)
        ...
        material_adaptation["surface_adjustment"] = surface_adjustment
        material_adaptation["hsv_adjusted_materials"] = int(hsv_adjusted_materials)
        material_adaptation["hsv_adjustment"] = {
            **hsv_adjustment,
            "saturation_scale": round(float(hsv_adjustment["saturation_scale"]), 4),
            "value_scale": round(float(hsv_adjustment["value_scale"]), 4),
            "hue_offset": round(float(hsv_adjustment["hue_offset"]), 4),
        }
