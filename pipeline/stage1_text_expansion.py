...
from research_prior_utils import (
...
def build_scene_conditioned_prompt(
    concept_name: str,
    scene_profile: dict,
    ...
    if research_guidance:
        guidance_items = []
        for line in research_guidance.splitlines():
            line = line.strip()
            if not line.startswith("- "):
                continue
            if line.startswith("- prior_id:") or line.startswith("- tags:"):
                continue
            guidance_items.append(line[2:].strip())
        compact_guidance = " ".join(guidance_items)[:900]
        guidance_sentence = (
            "Use this research prior as advisory context while preserving the hard single-object "
            f"white-background constraints: {compact_guidance} "
        )
    prompt = (
...
def main():
    ...
    parser.add_argument("--research-prior-path", default=None,
    ...
    research_prior = load_research_prior(args.research_prior_path, "[Stage 1]")
    research_guidance = build_guidance_block(research_prior, audience="prompt")
    if research_guidance:
        print(
            "[Stage 1] Loaded research prior: "
            f"{research_prior.get('prior_id') or research_prior.get('id') or args.research_prior_path}"
        )
    elif args.research_prior_path:
        print("[Stage 1] WARN: research prior provided but no prompt guidance was found")
    ...
    if repair_spec is not None:
        ...
        output_file.write_text(json.dumps(attach_prior_metadata([repaired], research_prior), indent=2, ensure_ascii=False))
    ...
    if args.dry_run:
        ...
        results = attach_prior_metadata(results, research_prior)
    ...
    if args.template_only or args.scene_conditioned:
        ...
        results = []
        for obj_id, name, category in selected:
            expanded = build_scene_conditioned_prompt(name, SCENE_PROFILE_4BLEND, research_guidance)
            results.append({
                "id": obj_id,
                "name": name,
                "category": category,
