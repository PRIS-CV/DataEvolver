...
#
# Usage:
#   bash src/dataevolver/cli/run_all.sh                  # Run all stages
#   bash src/dataevolver/cli/run_all.sh --from-stage 2   # Resume from Stage 2
...
DATA_DIR="${DATAEVOLVER_RUNTIME_ROOT:-${REPO_ROOT}/.dataevolver/runtime}"
LOG_DIR="${DATA_DIR}/logs"
PYTHON="${PYTHON:-python3}"
BLENDER_BIN="${BLENDER_BIN:-blender}"
export PYTHONPATH="${SRC_ROOT}${PYTHONPATH:+:${PYTHONPATH}}"
export DATAEVOLVER_RUNTIME_ROOT="${DATA_DIR}"
...
build_stage1_cmd() {
    ...
    if [[ "$FORCE_STAGE1" == "1" ]]; then
        echo "$PYTHON -m dataevolver.workflows.stages.text_expansion"
        return 0
    fi
...
run_stage 2 "Text-to-Image (Qwen-Image-2512)" \
    "$PYTHON -m dataevolver.workflows.stages.t2i_generate --device $DEVICE"

# ── Stage 2.5: SAM2 Foreground Segmentation ───────────────────────────────────
run_stage 2.5 "SAM2 Foreground Segmentation" \
    "$PYTHON -m dataevolver.workflows.stages.sam_segment --device $DEVICE"

# ── Stage 3: Image-to-3D ─────────────────────────────────────────────────────
run_stage 3 "Image-to-3D (Hunyuan3D-2.1)" \
    "$PYTHON -m dataevolver.workflows.stages.image_to_3d --device $DEVICE && mkdir -p \"${DATA_DIR}/meshes\" && find \"${DATA_DIR}/meshes_raw\" -maxdepth 1 -type f -name '*.glb' -exec cp -f {} \"${DATA_DIR}/meshes/\" \\; && echo '[run_all] Stage 3 canonical meshes refreshed from meshes_raw/'"
...
run_stage 5 "Merge Metadata & Generate CSVs" \
    "$PYTHON -m dataevolver.dataset.merge_metadata"
