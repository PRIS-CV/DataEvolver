...
BLENDER_SCRIPT="${REPO_ROOT}/src/dataevolver/workflows/stages/blender_render.py"
DATA_DIR="${DATAEVOLVER_RUNTIME_ROOT:-${REPO_ROOT}/.dataevolver/runtime}"
INPUT_DIR="${DATA_DIR}/meshes"
OUTPUT_DIR="${DATA_DIR}/renders"
...
# ── Validate ──────────────────────────────────────────────────────────────────
if [[ "$BLENDER_BIN" == */* && ! -f "$BLENDER_BIN" ]]; then
    echo "[ERROR] Blender not found at $BLENDER_BIN"
    echo "  Set BLENDER_BIN=/path/to/blender"
    exit 1
fi
if [[ "$BLENDER_BIN" != */* ]] && ! command -v "$BLENDER_BIN" >/dev/null 2>&1; then
    echo "[ERROR] Blender command not found: $BLENDER_BIN"
    echo "  Set BLENDER_BIN=/path/to/blender"
    exit 1
fi
