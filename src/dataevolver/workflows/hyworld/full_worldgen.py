...
from dataevolver.paths import REPO_ROOT
...
from dataevolver.runtime.hyworld_scene_contract import build_scene_contract, write_contract
...
def load_runtime(args: argparse.Namespace) -> tuple[dict[str, str], Path | None, dict[str, Any] | None]:
    ...
    if args.profile:
        try:
            from dataevolver.runtime.production_runtime import (
            ...
            raise RuntimeError(
                "--profile requires src/dataevolver/production_runtime.py or an installed production runtime module"
            ) from exc
