...
from dataevolver.paths import REPO_ROOT, SRC_ROOT
    ...
    def generate(self, request: DatasetRequest, output_root: Path) -> List[DatasetSample]:
        ...
        if not (request.skip_existing and all(existed_before.values())):
            cmd = [
                sys.executable,
                "-m",
                "dataevolver.workflows.stages.t2i_generate",
                "--prompts-path",
                str(prompt_path),
                "--output-dir",
                str(image_dir),
            ...
            if result.returncode != 0:
                raise RuntimeError(
                    "dataevolver.workflows.stages.t2i_generate failed "
                    f"with code {result.returncode}; see {log_dir}"
                )
        else:
        ...
        for row in prompt_rows:
            ...
            samples.append(
                DatasetSample(
                    ...
                    metadata={
                        "dry_run": False,
                        "backend": "dataevolver.workflows.stages.t2i_generate",
